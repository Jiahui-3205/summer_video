...
def train(epochs, batch_size,width, height,upscale,generator_optimizer,patch_size,group_size):
    ...
    for i in range (epochs):
      ...
      for j in range (batch_count):
        ...
        for k in range (batch_size):
          ...
          for m in range (skip_frame):
            if mod-(m+1)<0:
              front = cv2.imread('./LR/X4/%s/%s.png'%(group_number,'00000000'))/255
              back_num=zeropad((mod+(m+1)),8)
              back = cv2.imread('./LR/X4/%s/%s.png'%(group_number,back_num))/255
            elif mod+(m+1)>99:
              front_num=zeropad((mod-(m+1)),8)
              front = cv2.imread('./LR/X4/%s/%s.png'%(group_number,front_num))/255
              back = cv2.imread('./LR/X4/%s/%s.png'%(group_number,'00000099'))/255
            else:
              front_num=zeropad((mod-(m+1)),8)
              front = cv2.imread('./LR/X4/%s/%s.png'%(group_number,front_num))/255
              back_num=zeropad((mod+(m+1)),8)
      ...
      pic=(show[0,:,:,:])*255
      cv2.imwrite("./test_pic/epoch%d.jpg"%(i),pic)

      


train(100,1,180,320,4,generator_optimizer,7,30) 
